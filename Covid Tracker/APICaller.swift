...
extension DateFormatter {
    static let dayFormatter: DateFormatter = {
        ...
        formatter.dateFormat = "YYYY-MM-dd"    //the block is written correct for both timer
    ...
    static let prettyFormater: DateFormatter = {
        let formatter = DateFormatter()
        formatter.dateStyle = .medium    //the block is written correct for both timer
        formatter.timeZone = .current
        formatter.locale = .current
        return formatter
...
class APICaller {
    ...
    public func getCovidData(
        ...
        case .state(let state):
            urlString =
                "https://api.covidtracking.com/v2/states/\(state.state_code.lowercased())/daily.json"
        }
            ...
            let task = URLSession.shared.dataTask(with: url) { data, _, error in
                ...
                do {
                    ...
                    let models: [DayData] = result.data.compactMap {
                        guard let value = $0.cases.value, //time 00:37:20 time for error correction needed
